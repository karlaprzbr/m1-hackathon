...
use QuantumCloud\wpvoicemessage as wpvoicemessage;
...
final class UI {
	...
	public function render_chosen_dropdown( $options, $selected = [], $helper_text = '', $attributes = [] ) {
		...
		$class = isset( $attributes['class'] ) ? $attributes['class'] : '';
		$class = 'vmwpmdp-chosen chosen-select ' . $class;
		$class = trim( $class );
	...
	**/
                    ...
                    <ul class="mdc-list mdc-select-width">
                        <?php foreach ( $options as $key => $value ) : ?>
                            ...
                            <li class="mdc-list-item <?php esc_attr_e( $selected_class ); ?>" data-value="<?php esc_attr_e( $key ); ?>" role='option'>
                                <?php $img_file = wpvoicemessage::$path . 'images/' . $key . '.svg'; ?>
                                <?php if ( file_exists( $img_file ) ) : ?>
                                    <img src="<?php echo esc_url( wpvoicemessage::$url . "images/" . $key . ".svg" ); ?>" alt="<?php echo esc_attr( $key ); ?>" >
                                <?php endif; ?>
	...
	public function render_icon( $value, $label = '', $helper_text = '', $attributes = [], $meta = [] ) {
		...
		/** Get icon folder URL. */
		$icon_folder_url = wpvoicemessage::$url . 'images/mdc-icons/';
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                ><?php esc_html_e( 'Icon Library', 'wpvoicemessage' ); ?></button> -->
                <button style="width: 100%;" class="mdc-icon-field-upload-btn mdc-button mdc-button--outlined mdc-ripple-upgraded"><?php esc_html_e( 'Upload Icon', 'wpvoicemessage' ); ?></button>
            </div>
